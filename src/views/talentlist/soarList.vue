 ...
 * @LastEditTime: 2021-01-25 11:47:47
 * @Describe: 飙升榜
-->
<template>
    ...
    <div class="talentfind-block tab-block">
       <el-form ref="talentform" :model="talentform">
        ...
        <el-form-item class="cont-set">
              <el-select v-model="talentform.selectTm" placeholder="请选择" size="mini" class="cont-set_select">
              <el-option 
                v-for="item in talentform.selectTmoptions"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
  ...
  <el-table
      ...
      :default-sort = "{prop: 'follower_grow', order: 'descending'}"
      :height="tableHeight"
      :data="talentData"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div class="taccount">
            <span class="tit-nor">{{scope.row.follower | numtransfor}}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        ...
        label="点赞总量">
         <template slot-scope="scope">
          <div class="taccount">
            <span class="tit-nor">{{scope.row.favorited | numtransfor}}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <div class="taccount">
            <span class="taccount-one">{{scope.row.follower_grow | numtransfor}}</span>&nbsp;
            <span v-if="scope.row.follower_grow_rate >= 0" class="taccount-two upset">{{scope.row.follower_grow_rate | pretransfor}}↑</span>
            <span v-else class="taccount-two downset">{{scope.row.follower_grow_rate | pretransfor}}↓</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        ...
        label-class-name=yzordertit
        :sort-orders="['descending']"
        :sort-by="['favorited_grow']"
        sortable
        label="点赞增量">
        <template slot-scope="scope">
          <div class="taccount">
            <span class="taccount-one">{{scope.row.favorited_grow | numtransfor}}</span>&nbsp;
            <span v-if="scope.row.favorited_grow_rate >= 0" class="taccount-two upset">{{scope.row.favorited_grow_rate | pretransfor}}↑</span>
            <span v-else class="taccount-two downset">{{scope.row.favorited_grow_rate | pretransfor}}↓</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <div class="taccount">
            <span class="tit-font">{{scope.row.grow_rate | yznumtransfor}}</span>&nbsp;
          </div>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
import { gettalentsoarList } from '@/api/locowerApi';
...
export default {
  ...
  methods: {
    ...
    getTalentlt(arr) {
      ...
      // 排序
      if (arr !== undefined) {
        if (arr.prop === 'favorited_grow') {
          params.orderBy = 1;
        }
        if (arr.prop === 'grow_rate') {
          params.orderBy = 2;
        }
      }
      this.loading = true;
      gettalentsoarList(params).then((res) => {
        if (res.code === 200) {
