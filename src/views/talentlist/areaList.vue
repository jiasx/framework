 ...
 * @LastEditTime: 2021-01-25 11:50:02
 * @Describe: 地域榜
-->
<template>
    ...
    <div class="talentfind-block tab-block">
       <el-form ref="talentform" :model="talentform">
        <el-form-item label="地区">
          <el-radio-group v-model="talentform.checkedareatype" size="mini" @change="selectData">
            <el-radio-button v-for="item in areatypeOptions" :key="item.id" :label="item.id">
              {{ item.name }}
            </el-radio-button>
          </el-radio-group>
        </el-form-item>
...
<script>
import { gettalentareaList } from '@/api/locowerApi';

export default {
  name: 'areaList',
  data() {
    return {
      tableHeight: (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) - 330,
      is_show: true,
      // 达人发现数据
      talentData: [],
      // 接口时间初始化
  ...
  mounted() {
    ...
    window.onresize = () => (() => {
      ...
      that.tableHeight = window.tableHeight - 330;
    })();
  },
  computed: {
    wordtit() {
  ...
  methods: {
    ...
    getTalentlt(arr) {
      const params = {
        ...
        // 地区
        location: this.talentform.checkedareatype,
      };
      // 排序
      if (arr !== undefined) {
      ...
      this.loading = true;
      gettalentareaList(params).then((res) => {
        if (res.code === 200) {
